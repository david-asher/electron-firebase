...
/**
 * Works like the deprecated node.js url.format() function, but no auth.
 * @param {*} urlObject - A URL object (as returned by url.parse() or constructed otherwise). 
 * @returns {String} A formatted URL string derived from urlObject.
 * @alias module:applib
 * @see {@link https://nodejs.org/api/url.html#url_url_format_urlobject}
 */
function urlFormat( urlObject )
{
    var newUrl = ( urlObject.protocol || "https" ) + "://"
    if ( urlObject.host ) newUrl += urlObject.host
    else if ( urlObject.hostname ) {
        newUrl += urlObject.hostname
        if ( urlObject.port ) newUrl += ":" + urlObject.port
    }
    else return null // error, no host
    if ( urlObject.pathname ) newUrl += "/" + urlObject.pathname
    if ( !urlObject.query ) return newUrl
    return newUrl + "?" + querystring.stringify( urlObject.query )
}
...
function request( options )
{
    ...
    // electron should look like chrome when making API requests
    if ( !reqOptions.headers ) reqOptions.headers = {}
    reqOptions.headers['User-Agent'] = global.userAgent
