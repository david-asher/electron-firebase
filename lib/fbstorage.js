...
class fileStore
{
    ...
    async _unionFileDb( foldername )
    {
        ...
        console.info( "_unionFileDb: ", foldername )
    ...
    async _removeFolderDb( folderPath )
    {
        ...
        try {
            ...
            console.info( "_removeFolderDb: ", folderPath, folderList )
    ...
    async _initFolderList()
    {
        var result = null
        try {
            // build the list of folder names by querying the file set in firestore
            const folderNames = {}
            const querySnapshot = await this._findFileDb( "folder", "", "!=" )
            await querySnapshot.docs.forEach( async (snapshot) => {
                folderNames[ snapshot.data().folder ] = true
            })
            // write the folder list to  /.FileSet/.FolderList 
            const updateMap = {}
            updateMap[ folderArray ] = Object.keys( folderNames )
