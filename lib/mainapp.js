...
const folderConfigOpen   = "config"
const folderConfigDev    = "developer"
const folderConfigSecret = "transfer"
...
function getConfigFile( filename, foldername, bIsSecret ) 
{
    // reads the config JSON file, returning null if the file doesn't exist or gets a read error
    // if it's marked as a secret file, it will be deleted after reading
    // either way, any valid JSON content will be returned (synchronous)
    const configFileName = `${process.env.INIT_CWD}/${foldername}/${filename}.json`
    if ( !efb.file.isFile( configFileName ) ) return null
    const configContent = efb.file.readJSON( configFileName )
    if ( bIsSecret ) efb.file.deleteFile( configFileName )
    return configContent
}

async function getConfigContent( filename, secretKeyName ) 
{
    /*
    // if the app has stored secrets, then only allow config updates from /transfer
    const secretStored = await efb.secrets.getSecret( secretKeyName )
    const configTransfer = getConfigFile( filename, "transfer", true )
    if ( secretStored && configTransfer ) {
        await efb.secrets.setSecret( secretKeyName, configTransfer )
...
*/
    // config files in /config and /developer are not saved to secret store
    return getConfigFile( filename, "developer" ) || getConfigFile( filename, "config" )
}

/**
 * Must be called before other APIs. Reads the configuration files, keeps each 
 * configuration content in the secret store and deletes the original files. In 
 * this way, the config files are used to transfer information to the app one time 
 * only. App updates can include changed config files which will update the secret 
 * store. Each config file content is stored in a global variable and frozen. 
 * @alias module:mainapp
 */
async function setupAppConfig()
{
    try {
        const nodePackage = efb.file.readJSON( process.env.npm_package_json )

        // read the firebase configuration file
        global.fbConfig = await getConfigContent( "firebase-config", "fbConfig" )
        Object.freeze( global.fbConfig )

        // read the app configuration file
        global.appConfig = await getConfigContent( "app-config", "appConfig" )
        Object.freeze( global.appConfig )

        // read the Content Security Policy and build a string from the structure
        const cspConfig = await getConfigContent( "content-security-policy", "cspConfig" )
        global.ContentSecurityString = Object.entries( cspConfig ).map( ([key,value]) => {
            return key + " " + value.join( " " )
        }).join( ";" )
        Object.freeze( global.ContentSecurityString )
...
function sendToBrowser( channel, payload )
{
    ...
    try {
        ...
        console.error( "sendToBrowser error: ", error )
    }
}
...
function apiGetFirebaseConfig( req, res, next )
{
    ...
    const configSet = { ...global.fbConfig, providers: appConfig.providers }
...
async function startMainApp( options )
{
    ...
    try {
        // helpful, 'cuz this stuff happens
