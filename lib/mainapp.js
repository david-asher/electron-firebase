...
{
    var content = null
    const developerFileName = `${process.env.INIT_CWD}/developer/${filename}.json`
    const configFileName = `${process.env.INIT_CWD}/config/${filename}.json`
    if ( efb.file.isFile( developerFileName ) ) {
        content = efb.file.readJSON( developerFileName )
    }
    else if ( efb.file.isFile( configFileName ) ) {
        content = efb.file.readJSON( configFileName )
    }
    return content
}
...
async function setupAppConfig()
{
    try {
        ...
        console.log( "app package = ", nodePackage )
...
console.log( "fbConfig = ", await efb.secrets.getSecret( 'fbConfig') )


console.log( "app path = ", app.getPath('userData') )
await efb.secrets.setSecret( 'fbConfig', global.fbConfig )
await efb.secrets.setSecret( 'appConfig', global.appConfig )
await efb.secrets.setSecret( 'ContentSecurityPolicy', global.ContentSecurityPolicy )
