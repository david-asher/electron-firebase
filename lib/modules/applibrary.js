...
async function request( options )
{
    ...
    // convert request options format to got
    const gotOptions = { ...options }
    if ( typeof options.url !== 'string' ) gotOptions.url = urlFormat( gotOptions.url )
    if ( options.qs ) gotOptions.searchParams = gotOptions.qs
    if ( options.strictSSL ) gotOptions.rejectUnauthorized = gotOptions.strictSSL 
    if ( options.gzip ) gotOptions.decompress = gotOptions.gzip
    if ( options.jar ) gotOptions.cookieJar = gotOptions.jar
    if ( options.json && typeof options.json == 'boolean' && options.body ) {
        gotOptions.json = options.body
        delete gotOptions.body
    }
    delete gotOptions.qs
    delete gotOptions.strictSSL
    delete gotOptions.gzip
    delete gotOptions.jar
    // got is already a promise, returning the HTTP response object
    return await webRequest( gotOptions )
}
