...
console.log( "gcpApi requestOptions: ", requestOptions )
    ...
    try {
        ...
        if ( response.statusCode >= 400 ) {
            throw( { error: response.statusMessage, code: response.statusCode, message: response.body } )
        }

console.log( "gcpApi RESPONSE: ", response.statusCode, response.statusMessage, response.body )
...
function startNewSignIn( bSignOutUser )
{
    try {
        ...
        const userAgent = global.mainWindow.webContents.getUserAgent()
        // global.mainWindow.webContents.getUserAgent()
        // global.appContext.userAgent
        const urlOptions = {
