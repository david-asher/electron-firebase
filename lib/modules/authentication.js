...
async function gcpApi( requestOptions )
{
    try {
        ...
        if ( status >= 400 ) {
            return { error: response.statusMessage, code: status, message: body }
        }
        return applib.isJSON( body ) ? applib.parseJSON( body ) : body
    }
    catch (error) {
...
function startNewSignIn( bSignOutUser )
{
    try {
        ...
        const urlOptions = {
            userAgent: userAgent.replace( /Electron[^\s]+/i, "" )
