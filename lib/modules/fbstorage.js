...
class fileStore
{
    ...
    _updateFileDb( filename, contents )
    {
        ...
        return this.root.write( this._urlPath( filename ), contents )
    }

    _appendFileDb( filename, arrayName, newValue )
    {
        return this.root.append( this._urlPath( filename ), arrayName, newValue, true )
    }
    ...
    {
        try {
            const folderMap = await this.root.read( this._dbPath( folderListDoc) )
            return folderMap[ folderArray ].filter( (element) => {
...
/***************************************************************
 * CHECK ALL _dbPath references - store path as unencoded
 ***************************************************************
 */
