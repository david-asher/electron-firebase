...
class fileStore
{
    ...
    _dbPath( filename )
    {
        ...
        return `${fileCollection}/${encodeURIComponent( filename )}`
    }
    ...
    _findFileDb( property, criteria, optionalMathOperator = "==" )
    {
        // returns a promise containing a QuerySnapshot
        // https://firebase.google.com/docs/reference/js/firebase.firestore.QuerySnapshot
        return this.root.find( fileCollection, property, criteria, optionalMathOperator )
    }

    _updateFileDb( filename, contents )
    {
        ...
        return this.root.write( this._dbPath( filename ), contents )
    }

    _appendFileDb( filename, arrayName, newValue )
    {
        return this.root.append( this._dbPath( filename ), arrayName, newValue, true )
    }

    _unionFileDb( foldername )
    {
        // keeps a list of unique folder names
        return this.root.union( this._dbPath( folderListDoc), folderArray, foldername )
    }

    async _removeFolderDb( folderPath )
    {
        // 
        await this.list( folderPath, true )
        .then( async (folderList) => {
            if ( !folderList || 0 == folderList.length ) return null
            return await this.root.remove( this._dbPath( folderListDoc), folderArray, folderPath )
        })
    }
    ...
    async _initFolderList()
    {
        try {
            ...
            await this.root.write( this._dbPath( folderListDoc ), updateMap )
            return true
        }
        catch (error) {
            console.error( "_initFolderList: ", error )
        }
    }
    ...
    async _newestDoc( docList, compareField )
    {
        ...
        return await docList.reduce( async ( accumulator, currentValue ) => {
    ...
    _isEmptyResponse( subject )
    {
        if ( !subject ) return true
        if ( typeof subject === 'string' && subject === "" ) return true
        if ( typeof subject === 'object' && Object.keys(subject).length === 0 ) return true
        return false
    }
    ...
    _metaFixup( fileMeta )
    {
        ...
        const pathURL = this._objectPathUrl( metaDB.path )
        metaDB.downloadUrl = `https://${pathURL.hostname}${pathURL.pathname}?alt=media&token=${fileMeta.downloadTokens}`
    ...
    _makeUploadOptions( filepath, contents )
    {
        ...
        const options = {
            ...
            qs: { name: this._pathPrefix( filepath ) },
            json: true
        }
    ...
    async find( filepath, queryMatch = "path" )
    {
        ...
        const querySnapshot = await this._findFileDb( queryMatch, filepath )
        if ( querySnapshot.empty ) {
            return null
        }
        // if the doc exists (i.e. matches the path) then we've found the doc
        return this._newestDoc( querySnapshot.docs, "updated" )
    }
    ...
    async upload( filepath, content )
    {
        ...
        const metaResult = this._metaFixup( fileMeta )
    ...
    {
        try {
            // check for existence and get the meta
            const metafile = await this.about( filepath )
            ...
            return await this._removeFolderDb( metafile.folder )
        }
        catch (error) {
            console.error( error )
        }
