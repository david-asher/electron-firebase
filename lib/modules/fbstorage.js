...
class fileStore
{
    ...
    _newestDoc( docList, compareField )
    {
        if ( !Array.isArray( docList ) ) return null
        if ( docList.length == 0 ) return null
        return docList.reduce( ( accumulator, currentValue ) => {
            const currentField = currentValue.get( compareField )
            const accumulatedField = accumulator.get( compareField )
    ...
    _metaFixup( fileMeta )
    {
        ...
        const pathURL = _objectPathUrl( metaDB.path )
 ...
 * * .global - File access without restrictions 
...
function initialize()
{
    ...
    storageSet.global = new fileStore( firestore.global, "global", "apps/global" )
