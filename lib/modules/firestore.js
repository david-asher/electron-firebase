...
class firestoreDoc
{
    ...
    async read( docPath, bGetFromServer = false )
    {
        ...
        try {
            ...
            console.error( "firestore.js read: ", docPath, error )
        }
        return result
    }
    ...
    async query( collectionPath, fieldName, fieldMatch, matchOperator = "==" )
    {
        ...
        if ( this.cacheValid ) {
            authn.firestore().disableNetwork()
            this.cacheValid = true
        }
        try {
            ...
            result = await resultQuery.get() || result
            // if we got an error, enable network to go directly to the server
            if ( !result || result.empty ) {
                authn.firestore().enableNetwork()
                result = await resultQuery.get() || result
            }
        }
        catch (error) {
