...
class firestoreDoc
{
    ...
    _ref( docPath )
    {
        ...
        try {
            ...
            docRef = collectionRef.doc( 0 === ( nParts % 2 ) ? docName : "" )
        }
        catch (error) {
            console.error( "_ref: ", docPath, error )
        }
        return docRef
    }
     ...
     */
    about( docPath, bGetFromServer = false )
    {
        ...
        try {
            ...
            result = docRef.get( this._fromServer( bGetFromServer ) )
        }
        catch (error) {
            console.error( "about: ", docPath, error )
        }
        return result
    }
    ...
    async write( docPath, contents = {} )
    {
        ...
        try {
            const docRef = this._ref( docPath )
            var snap = await docRef.get( this._fromServer( true ) )
            await docRef.set( contents, snap.exists ? setMergeFields : setCreateFields )
            const getSnap = await docRef.get()
            result = await getSnap.data()
        }
        catch( error ) {
            console.log( "write: ", docPath, error )
        }
        return result
    }
    ...
    async read( docPath, bGetFromServer = false )
    {
        ...
        try {
            const docRef = this._ref( docPath )
            const snap = await docRef.get( this._fromServer( bGetFromServer ) )
            result = await snap.data( getSnaphotOptions )
        }
        catch( error ) {
            console.log( "read: ", docPath, error )
        }
        return result
    }
    ...
    async field( docPath, fieldName, bGetFromServer = false )
    {
        ...
        try {
            const docRef = this._ref( docPath )
            const snap = await docRef.get( this._fromServer( bGetFromServer ) )
            result = await snap.get( fieldName )
        }
        catch( error ) {
            console.log( "field: ", docPath, fieldName, error )
        }
        return result
    }
    ...
    async delete( docPath )
    {
        var result = null
        try {
            result = this._ref( docPath ).delete()
        }
        catch (error) {
            console.error( "delete: ", docPath, error )
        }
        return result
    }
    ...
    async find( collectionPath, fieldName, fieldMatch, matchOperator = "==" )
    {
        ...
        try {
            ...
            result = collectionRef.where( fieldName, matchOperator, fieldMatch ).get()
        }
        catch (error) {
            console.error( "find: ", collectionPath, fieldName, fieldMatch, error )
        }
        return result
    }
    ...
    async union( docPath, arrayName, newValue )
    {
        ...
        try {
            ...
            result = await this._ref( docPath ).update( updateElement )
        }
        catch (error) {
            console.error( "union: ", docPath, arrayName, newValue, error )
        }
        return result
    }
    ...
    async remove( docPath, arrayName, oldValue )
    {
        ...
        try {
            ...
            console.error( "remove: ", docPath, arrayName, oldValue, error )
        }
        return result
    }
    ...
    async append( docPath, arrayName, newValue, bGetFromServer = false )
    {
        ...
        try {
            ...
            console.error( "append: ", docPath, arrayName, newValue, error )
        }
        return result
    }
    ...
    async splice( docPath, arrayName, oldValue, bGetFromServer = false )
    {
        ...
        try {
            const docRef = this._ref( docPath )
            const snap = docRef.get( this._fromServer( bGetFromServer ) )
            var baseDoc = snap.data()
            var arrayRef = baseDoc[ arrayName ]
            ...
            console.error( "splice: ", docPath, arrayName, oldValue, error )
        }
        return result
    }
}
...
function initialize( userid, projectId )
{
    try {
        ...
        console.error( "firestore initialize: ", error )
    }
}
