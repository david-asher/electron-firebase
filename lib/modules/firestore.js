...
class firestoreDoc
{
    ...
    async read( docPath, bGetFromServer = false )
    {
        var result = null
        try {
            const readResult = await this.about( docPath, bGetFromServer )
     ...
     * @alias module:firestore
     */
    async push( docPath, arrayName, newValue, bGetFromServer = false )
    {
        const docRef = this._ref( docPath )
        const snap = await docRef.get( fromServer( bGetFromServer ) )
     ...
     * @alias module:firestore
     */
    async pop( docPath, arrayName, bGetFromServer = false )
    {
        var popped = null
        const docRef = this._ref( docPath )
        const snap = await docRef.get( fromServer( bGetFromServer ) )
 ...
 * cache to the cloud.
 * 
 * @param {boolean} bForceCache - If true, stop the network connection and force cached operations
 * @alias module:firestore
 */
