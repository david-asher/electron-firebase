 ...
 * @param {BrowserWindow} setPrimaryWindow - The primary application window that will be kept open
 * @param {ipcMain} setMainIpc - The main process IPC connection
 * @see {@link https://electronjs.org/docs/api/browser-window|BrowserWindow}
 * @see {@link https://electronjs.org/docs/api/ipc-main|ipcMain}
 * @alias module:local
 */
function setup( setPrimaryWindow, setMainIpc )
{
    openWindow = setPrimaryWindow
    mainIpc = setMainIpc
}
...
function _send( topic, key, value )
{
    try {
        ...
        const bad = openWindow.id
        openWindow.webContents.send( "localStorage", topic, key, value )
    }
    catch (error) {
 ...
 */
function _receive( topic )
{
    return new Promise( (resolve,reject) => {
        if ( !mainIpc ) return
        mainIpc.once( topic, ( event, value ) => {
            try {
...
function setItem( key, value ) 
{
    if ( value && typeof value == 'object' ) {
        try {
            ...
            console.error( "JSON stringify error: ", error, ", key = ", key, ", value = ", value )
        }
    }
