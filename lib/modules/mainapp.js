...
function signoutUser() 
{
    ...
    dialog.showMessageBox( global.mainWindow.window(), dlgOptions, ( response, checkboxChecked ) => {
        if ( response != 1 ) return
        efb.auth.signOutUser( global.mainWindow )
        .then( () => {
            if ( checkboxChecked ) efb.auth.signOutProvider( provider, global.mainWindow )
            efb.applib.event.emit( "user-signout" )
        })
        .catch( (error) => {
            dialog.showErrorBox( "ERROR on usersignout:", error )
        })
        .finally( () => {
