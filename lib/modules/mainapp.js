 ...
 * @returns {object} The global.appContext object
 * @alias module:mainapp
 */
...
async function getBrowserInfo()
{
    return new Promise( (resolve, reject ) => 
    {
        ...
        ipcMain.once( 'about-browser', (appEvent,...args) => {
            _catchBrowserInfo.apply( this, args )

console.log( "createWindow getBrowserInfo ", Date.now() - global.lastStamp, args  )
 ...
 * @param {object} options - May contain show, width, height, title, open_html
 * @returns {Promise<object>} The new electron main window
 * @alias module:mainapp
 */
...
console.log( "createWindow efb.window.open ", Date.now() - global.lastStamp  )
...
console.log( "startMainApp await getBrowserInfo() ", Date.now() - global.lastStamp )

            // The mainWindow browser will report context information back to Main process
            // await 
            ipcMain.once( 'about-browser', (appEvent,...args) => {

console.log( "createWindow getBrowserInfo ", Date.now() - global.lastStamp  )

                _catchBrowserInfo.apply( this, args )
            })
        }
        
console.log( "createWindow newWindow.once close ", Date.now() - global.lastStamp  )
        ...
        newWindow.once( 'close', (event) => 
        {
            ...
            windowList.find( (element, index) => {
                if ( element !== newWindow ) return false
...
//////        await newWindow.waitForShow()
console.log( "createWindow newWindow.waitForShow() ", Date.now() - global.lastStamp  )

        // await 
        newWindow.waitForShow()
        .then( () => {
console.log( "createWindow newWindow.resize ", Date.now() - global.lastStamp  )
            newWindow.resize( () => {

console.log( "createWindow openOptions.show ", Date.now() - global.lastStamp, openOptions.show )
  ...
  /*      
        // resize the window from the previous session, which must be after the window is ready
        // because the window bounds are remembered in the browser LocalStorage
        newWindow.resize( () => {

console.log( "createWindow windowOptions.show ", Date.now() - global.lastStamp, windowOptions.show )

            if ( windowOptions.show ) newWindow.show()
            efb.applib.event.emit( "window-open", newWindow )
        })
*/
console.log( "createWindow return newWindow ", Date.now() - global.lastStamp  )
 ...
 * @alias module:mainapp
 */
function _catchBrowserInfo( browserInfo )
{
...
console.log( "startMainApp efb.auth.initializeFirebase ", Date.now() - global.lastStamp )
...
console.log( "startMainApp startWebServices ", Date.now() - global.lastStamp )

        // the web server is used for secure communication with the BrowserWindow
        await startWebServices()

console.log( "startMainApp await createWindow ", Date.now() - global.lastStamp )

        // open the first window which will be the global.main window
        const mainWindow = await createWindow( options )

console.log( "startMainApp await efb.auth.startNewSignIn ", Date.now() - global.lastStamp )
