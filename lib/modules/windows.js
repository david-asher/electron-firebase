...
console.log( "waitForShow START ", Date.now() - global.lastStamp )
...
console.log( "waitForShow resolve ", Date.now() - global.lastStamp )
...
console.log( "resize START ", Date.now() - global.lastStamp )

        var updatedBounds = null
        if ( !this.boundsCheckerEnabled ) return 
        local.getItem( this.boundsKey )
        .then( ( bounds ) => {

console.log( "resize getItem ", Date.now() - global.lastStamp, bounds )
