...
class open extends BrowserWindow
{
    ...
    constructor( urlToOpen, setOptions, urlOptions )
    {
        ...
        const isLocalHost = ( "localhost" == urlParts.hostname )
        const baseOptions = {
            show: false,
            resizable: true,
            movable: true,
            webPreferences: {
                nodeIntegration: isLocalHost,
                enableRemoteModule: isLocalHost,
                contextIsolation: !isLocalHost
            }
        }
        ...
        // set the window's user agent to our definition
        if ( !urlOptions ) urlOptions = {}
        if ( global.userAgent && !urlOptions.userAgent ) urlOptions.userAgent = global.userAgent
  
        // just a notification when the window closes
        super.once( 'close', () => {
            // console.log( "WINDOW CLOSED: ", super.id, super.title )
        })
    ...
    _setupBoundsChecker( title )
    {
        ...
        this.on( 'resize', this._updateBounds )
        this.on( 'move', this._updateBounds ) 
    ...
    send( channel, payload )
    {
        try {
            ...
            console.error( "window send error: ", error )
        }
    }
